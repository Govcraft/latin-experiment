...
\textbf{Key findings}: Pressure-field coordination outperforms all baselines (\S\ref{sec:main-results}). Temporal decay is critical---disabling it reduces solve rate by 10 percentage points (\S\ref{sec:ablations}). The approach maintains consistent performance from 1 to 4 agents (\S\ref{sec:scaling}). Despite using more tokens per trial, pressure-field achieves 12\% better token efficiency per successful solve (\S\ref{sec:token-efficiency}).
